...
def view_data():
    ...
    with engine.begin() as conn:
        ...
        offset = (current_page - 1) * page_size  # Calculate the offset dynamically

        query = f"SELECT * FROM {DB_TABLE} WHERE delito = 'Homicidio' LIMIT {page_size} OFFSET {offset};"
        result_proxy = conn.execute(text(query))
        result = result_proxy.fetchall()

        # Fetch the column names from the ResultProxy object
        headers = result_proxy.keys()

        # Render the template with the data and headers
        return render_template('data.html', data=result, headers=headers, current_page=current_page)
